          ...
          <tbody>
            ...
            {filtrados.map(c => (
              <tr key={c.id}>
                ...
                <td style={styles.td}>
                  {/* Entrenador solo puede editar, no eliminar */}
...
function ModuloMembresias() {
  ...
  async function guardarMembresia() {
    ...
    const { data: entrenador } = await supabase.from('usuarios').select('id').eq('cui', cui).single()
    const { error } = await supabase.from('membresias').insert({ cliente_id: formMem.cliente_id, tipo_membresia_id: formMem.tipo_membresia_id || null, precio_pagado: precio, fecha_inicio: formMem.fecha_inicio, fecha_fin: formMem.fecha_fin, monto_pagado: monto, estado_pago, estado: 'activa', registrado_por: entrenador?.id || null })
    setLoading(false)
    if (error) { setMsg({ type: 'error', text: error.message }); return }
  ...
  return (
    <div>
      ...
      {tab === 'planes' && (
        <div style={styles.card}>
          <table style={styles.table}>
            <thead><tr><th style={styles.th}>Plan</th><th style={styles.th}>Duración</th><th style={styles.th}>Precio</th><th style={styles.th}>Descripción</th></tr></thead>
            <tbody>
              {tipos.length === 0 && <tr><td colSpan={4} style={{ ...styles.td, textAlign: 'center', color: C.muted }}>No hay planes creados</td></tr>}
              {tipos.map(t => (
                <tr key={t.id}>
          ...
          <p style={{ color: C.muted, fontSize: '13px', marginTop: '12px' }}>Solo el administrador puede crear o editar planes.</p>
        </div>
      )}
      {tab === 'asignadas' && (
        <div style={styles.card}>
...
// ══════════════════════════════════════════════════════════
// MÓDULO: PRODUCTOS (solo lectura + ventas)
// ══════════════════════════════════════════════════════════
function ModuloProductos() {
  ...
  return (
    <div>
      ...
      <p style={{ color: C.muted, fontSize: '13px', marginBottom: '16px' }}>Solo el administrador puede agregar o editar productos. Aquí puedes ver el inventario disponible.</p>
      <div style={{ ...styles.card, padding: '14px 20px' }}>
        <input style={styles.input} placeholder="🔍 Buscar producto..." value={busqueda} onChange={e => setBusqueda(e.target.value)} />
      </div>
      <div style={styles.card}>
        <table style={styles.table}>
          <thead><tr><th style={styles.th}>Producto</th><th style={styles.th}>Precio</th><th style={styles.th}>Stock</th><th style={styles.th}>Estado</th></tr></thead>
          <tbody>
            {filtrados.length === 0 && <tr><td colSpan={4} style={{ ...styles.td, textAlign: 'center', color: C.muted }}>No hay productos</td></tr>}
            {filtrados.map(p => (
              <tr key={p.id}>
...
function ModuloVentas() {
  ...
  async function registrarVenta() {
    ...
    const cui = sessionStorage.getItem('cui')
    const { data: entrenador } = await supabase.from('usuarios').select('id').eq('cui', cui).single()
    const { error } = await supabase.from('ventas').insert({ cliente_id: form.cliente_id, producto_id: form.producto_id, cantidad: parseInt(form.cantidad), precio_total: precioTotal, monto_pagado: monto, estado_pago, registrado_por: entrenador?.id || null })
    setLoading(false)
    if (error) { setMsg({ type: 'error', text: error.message }); return }
  ...
  async function registrarAbono() {
    ...
    const cui = sessionStorage.getItem('cui')
    const { data: entrenador } = await supabase.from('usuarios').select('id').eq('cui', cui).single()
    await supabase.from('abonos').insert({ venta_id: ventaSeleccionada.id, monto: parseFloat(abono), registrado_por: entrenador?.id || null })
          ...
          <tbody>
            ...
            {ventasFiltradas.map(v => (
              <tr key={v.id}>
                ...
                <td style={styles.td}>{v.estado_pago !== 'pagado' && <button style={{ ...styles.btn('success'), padding: '6px 12px', fontSize: '12px' }} onClick={() => { setVentaSeleccionada(v); setAbono(''); setModal('abono') }}>+ Abono</button>}</td>
              </tr>
            ))}
          </tbody>
        </table>
      ...
      {modal === 'venta' && (
        <div style={styles.modal} onClick={e => e.target === e.currentTarget && setModal(null)}>
          <div style={styles.modalBox}>
            ...
            {msg && <div style={styles.alert(msg.type)}>{msg.text}</div>}
            <div style={{ marginBottom: '14px' }}><label style={styles.label}>Cliente *</label><select style={styles.input} value={form.cliente_id} onChange={e => setForm({ ...form, cliente_id: e.target.value })}><option value="">— Seleccionar —</option>{clientes.map(c => <option key={c.id} value={c.id}>{c.nombre} {c.apellido_paterno} — {c.cui}</option>)}</select></div>
            <div style={{ marginBottom: '14px' }}><label style={styles.label}>Producto *</label><select style={styles.input} value={form.producto_id} onChange={e => setForm({ ...form, producto_id: e.target.value })}><option value="">— Seleccionar —</option>{productos.map(p => <option key={p.id} value={p.id}>{p.nombre} — Q{p.precio}</option>)}</select></div>
            <div style={styles.grid2}>
              ...
              {productoSeleccionado && <div style={{ gridColumn: '1 / -1', backgroundColor: C.surface, padding: '14px', borderRadius: '8px', fontSize: '14px' }}><div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '6px' }}><span style={{ color: C.muted }}>Total:</span><strong style={{ color: C.accent }}>Q{precioTotal.toFixed(2)}</strong></div><div style={{ display: 'flex', justifyContent: 'space-between' }}><span style={{ color: C.muted }}>Saldo:</span><strong style={{ color: C.yellow }}>Q{(precioTotal - parseFloat(form.monto_pagado || 0)).toFixed(2)}</strong></div></div>}
            </div>
            <div style={{ display: 'flex', gap: '10px', marginTop: '20px', justifyContent: 'flex-end' }}>
              <button style={styles.btn('secondary')} onClick={() => setModal(null)}>Cancelar</button>
        ...
        <div style={styles.modal} onClick={e => e.target === e.currentTarget && setModal(null)}>
          <div style={{ ...styles.modalBox, width: '400px' }}>
            ...
            <div style={{ backgroundColor: C.surface, padding: '14px', borderRadius: '8px', marginBottom: '16px', fontSize: '14px' }}>
              ...
              <div style={{ display: 'flex', justifyContent: 'space-between' }}><span>Saldo pendiente:</span><strong style={{ color: C.yellow }}>Q{(parseFloat(ventaSeleccionada.precio_total) - parseFloat(ventaSeleccionada.monto_pagado || 0)).toFixed(2)}</strong></div>
            </div>
            <div><label style={styles.label}>Monto del abono (Q)</label><input style={styles.input} type="number" step="0.01" value={abono} onChange={e => setAbono(e.target.value)} placeholder="0.00" autoFocus /></div>
            <div style={{ display: 'flex', gap: '10px', marginTop: '20px', justifyContent: 'flex-end' }}>
              <button style={styles.btn('secondary')} onClick={() => setModal(null)}>Cancelar</button>
...
function ModuloRutinas() {
  ...
  const [msg, setMsg] = useState(null)
  const [loading, setLoading] = useState(false)
  const [form, setForm] = useState({ cliente_id: '', titulo: '', contenido: '' })

  useEffect(() => { cargarTodo() }, [])

  async function cargarTodo() {
    const [{ data: r }, { data: c }] = await Promise.all([
      supabase.from('rutinas').select('*, cliente:cliente_id(nombre,apellido_paterno), entrenador:entrenador_id(nombre,apellido_paterno)').order('fecha_asignacion', { ascending: false }),
      supabase.from('usuarios').select('id,nombre,apellido_paterno,cui').eq('rol', 'cliente').order('nombre'),
    ])
    setRutinas(r || []); setClientes(c || [])
  }

  async function guardar() {
    if (!form.cliente_id || !form.titulo || !form.contenido) { setMsg({ type: 'error', text: 'Cliente, título y contenido son obligatorios' }); return }
    setLoading(true)
    const cui = sessionStorage.getItem('cui')
    const { data: entrenador } = await supabase.from('usuarios').select('id').eq('cui', cui).single()
    const op = seleccionado
      ? supabase.from('rutinas').update({ titulo: form.titulo, contenido: form.contenido, activa: true }).eq('id', seleccionado.id)
      : supabase.from('rutinas').insert({ cliente_id: form.cliente_id, entrenador_id: entrenador?.id || null, titulo: form.titulo, contenido: form.contenido })
    const { error } = await op
    setLoading(false)
    if (error) { setMsg({ type: 'error', text: error.message }); return }
    setMsg({ type: 'success', text: seleccionado ? 'Rutina actualizada' : 'Rutina asignada' })
    cargarTodo(); setTimeout(() => setModal(null), 1200)
  }
  ...
  return (
    <div>
      <div style={styles.pageHeader}>
        <h1 style={styles.pageTitle}>🏋️ Rutinas</h1>
        <button style={styles.btn('primary')} onClick={() => { setSeleccionado(null); setForm({ cliente_id: '', titulo: '', contenido: '' }); setMsg(null); setModal('rutina') }}>+ Nueva rutina</button>
      </div>
      <div style={styles.card}>
        <table style={styles.table}>
          <thead><tr><th style={styles.th}>Cliente</th><th style={styles.th}>Título</th><th style={styles.th}>Asignada por</th><th style={styles.th}>Fecha</th><th style={styles.th}>Estado</th><th style={styles.th}>Acciones</th></tr></thead>
          <tbody>
            {rutinas.length === 0 && <tr><td colSpan={6} style={{ ...styles.td, textAlign: 'center', color: C.muted }}>No hay rutinas asignadas</td></tr>}
            {rutinas.map(r => (
              <tr key={r.id}>
                <td style={styles.td}><strong>{r.cliente?.nombre} {r.cliente?.apellido_paterno}</strong></td>
                <td style={styles.td}>{r.titulo}</td>
                <td style={styles.td}>{r.entrenador?.nombre || '—'}</td>
                <td style={{ ...styles.td, fontSize: '12px', fontFamily: 'monospace' }}>{new Date(r.fecha_asignacion).toLocaleDateString()}</td>
                <td style={styles.td}>{r.activa ? <span style={styles.badge('green')}>Activa</span> : <span style={styles.badge('')}>Inactiva</span>}</td>
                <td style={styles.td}>
                  <div style={{ display: 'flex', gap: '8px' }}>
                    <button style={{ ...styles.btn('secondary'), padding: '6px 12px', fontSize: '12px' }} onClick={() => { setSeleccionado(r); setForm({ cliente_id: r.cliente_id, titulo: r.titulo, contenido: r.contenido }); setMsg(null); setModal('rutina') }}>Editar</button>
                    <button style={{ ...styles.btn('danger'), padding: '6px 12px', fontSize: '12px' }} onClick={() => eliminar(r)}>Eliminar</button>
                  </div>
                </td>
              </tr>
            ))}
      ...
      {modal === 'rutina' && (
        <div style={styles.modal} onClick={e => e.target === e.currentTarget && setModal(null)}>
          <div style={styles.modalBox}>
            <h2 style={{ margin: '0 0 20px', fontSize: '18px' }}>{seleccionado ? '✏️ Editar rutina' : '➕ Nueva rutina'}</h2>
            {msg && <div style={styles.alert(msg.type)}>{msg.text}</div>}
            <div style={{ marginBottom: '14px' }}>
              <label style={styles.label}>Cliente *</label>
              <select style={styles.input} value={form.cliente_id} disabled={!!seleccionado} onChange={e => setForm({ ...form, cliente_id: e.target.value })}>
            ...
            <div style={{ marginBottom: '14px' }}>
              ...
              <input style={styles.input} value={form.titulo} onChange={e => setForm({ ...form, titulo: e.target.value })} placeholder="Ej: Rutina de fuerza - Semana 1" />
            </div>
            <div style={{ marginBottom: '14px' }}>
              <label style={styles.label}>Contenido de la rutina *</label>
              <textarea style={{ ...styles.input, minHeight: '180px', resize: 'vertical', fontFamily: 'monospace', fontSize: '13px' }}
                value={form.contenido}
                onChange={e => setForm({ ...form, contenido: e.target.value })}
                placeholder={`Día 1 - Pecho y Tríceps:\n- Press banca: 4x10\n- Fondos: 3x12\n- Extensiones tríceps: 3x15\n\nDía 2 - Espalda y Bíceps:\n- Dominadas: 4x8\n- Remo: 3x10`}
              />
            </div>
            <div style={{ display: 'flex', gap: '10px', marginTop: '20px', justifyContent: 'flex-end' }}>
              <button style={styles.btn('secondary')} onClick={() => setModal(null)}>Cancelar</button>
              <button style={styles.btn('primary')} disabled={loading} onClick={guardar}>{loading ? 'Guardando...' : seleccionado ? 'Guardar cambios' : 'Asignar rutina'}</button>
            </div>
          </div>
        </div>
      )}
...
function ModuloDashboard() {
  const [stats, setStats] = useState({ clientes: 0, activas: 0, rutinas: 0, ventas: 0 })
  const nombre = sessionStorage.getItem('nombre')

  useEffect(() => {
    async function cargar() {
      const cui = sessionStorage.getItem('cui')
      const { data: yo } = await supabase.from('usuarios').select('id').eq('cui', cui).single()
      const [{ count: clientes }, { count: activas }, { count: rutinas }, { count: ventas }] = await Promise.all([
        ...
        supabase.from('rutinas').select('*', { count: 'exact', head: true }).eq('entrenador_id', yo?.id).eq('activa', true),
        supabase.from('ventas').select('*', { count: 'exact', head: true }).eq('registrado_por', yo?.id),
      ])
      setStats({ clientes: clientes || 0, activas: activas || 0, rutinas: rutinas || 0, ventas: ventas || 0 })
    }
    cargar()
  }, [])

  const cards = [
    { label: 'Clientes en el gimnasio', num: stats.clientes, icon: '👥', color: C.accent },
    { label: 'Membresías activas', num: stats.activas, icon: '✅', color: C.green },
    { label: 'Mis rutinas asignadas', num: stats.rutinas, icon: '🏋️', color: C.yellow },
    { label: 'Mis ventas registradas', num: stats.ventas, icon: '💰', color: C.red },
  ]
    ...
    <div>
      <div style={styles.pageHeader}>
        <div>
          ...
          <p style={{ margin: '4px 0 0', color: C.muted, fontSize: '14px' }}>Bienvenido, {nombre || 'Entrenador'}</p>
        </div>
      </div>
      <div style={styles.grid2}>
        {cards.map((c, i) => (
    ...
    <div style={styles.layout}>
      <aside style={styles.sidebar}>
        <div style={styles.logo}>
          ...
          <p style={styles.logoSub}>Panel Entrenador</p>
        </div>
        ...
        <div style={styles.sidebarFooter}>
          <div style={{ fontSize: '13px', color: C.muted, marginBottom: '10px' }}>👤 {nombre || 'Entrenador'}</div>
